...
public class HomeFragment extends Fragment {
    ...
    private void fetchAndDisplayBalances(ListView listView) {
        String url = "http://18.189.7.172/displayBills.php";
    ...
    private void fetchAndDisplayBills(ListView billsListView) {
        String url = "http://18.189.7.172/displayBills.php";  // URL to your PHP script

        StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                response -> {
                    try {
                        JSONArray billsArray = new JSONArray(response);
                        List<String> relevantBills = new ArrayList<>();
                        ...
                        for (int i = 0; i < billsArray.length(); i++) {
                            JSONObject bill = billsArray.getJSONObject(i);
                            String dueDateStr = bill.getString("due_date");
                            ...
                            if (dueDate != null && dueDate.compareTo(currentDate) >= 0) {
                                String billName = bill.getString("bill_name");
                                String amount = bill.getString("amount");
                                String formattedDueDate = new SimpleDateFormat("EEE, d MMM yyyy", Locale.getDefault()).format(dueDate);
                                relevantBills.add(billName + ": $" + amount + " (due by " + formattedDueDate + ")");
                            }
                        }

                        ArrayAdapter<String> billsAdapter = new ArrayAdapter<>(getActivity(), android.R.layout.simple_list_item_1, relevantBills);
                        billsListView.setAdapter(billsAdapter);
                    } catch (JSONException e) {
                        Toast.makeText(getActivity(), "Error parsing bills", Toast.LENGTH_LONG).show();
                    } catch (ParseException e) {
                        Toast.makeText(getActivity(), "Error parsing due date", Toast.LENGTH_LONG).show();
                    }
                },
                error -> Toast.makeText(getActivity(), "Error fetching bills: " + error.getMessage(), Toast.LENGTH_LONG).show()
        );
